import {
  ...
  const getContactData = () => {
    return async (dispatch) => {
      ...
      if (data) dispatch({type: GET_CONTACT_DATA_SUCCESS, payload: data});
    };
  };
  
  const updateContactData = (updatedData) => {
    return async (dispatch) => {
      dispatch({type: UPDATE_CONTACT_DATA, payload: updatedData});
    };
  };
  
  export {getContactData, updateContactData};
